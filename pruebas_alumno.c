...
pokedex_t *pokedex_con_3_pokemones()
{
	...
	if (!pokedex_insertar(pokedex, Pikachu) ||
...
void pruebas_pokedex_NULL()
{
	...
	pa2m_afirmar(pokedex_buscar_por_nombre(pokedex, "Pikachu") == NULL,
		     "buscar por nombre en pokedex NULL devuelve NULL");
	pa2m_afirmar(pokedex_buscar_por_id(pokedex, 1) == NULL,
		     "buscar por id en pokedex NULL devuelve NULL");

	pokedex = pokedex_crear();
	pa2m_afirmar(pokedex != NULL, "Se puede crear una pokedex vacia");

	pa2m_afirmar(pokedex_buscar_por_nombre(pokedex, NULL) == NULL,
		     "Buscar por nombre con nombre NULL devuelve NULL");
	pokedex_mostrar_por_nombre(NULL);
	pokedex_mostrar_por_id(NULL);
	pa2m_afirmar(true,
		     "Mostrar por nombre/id con pokedex NULL no trae errores");
	pokedex_destruir(pokedex);
}
...
void pruebas_pokedex_insertar()
{
	...
	pa2m_afirmar(
		pokedex != NULL,
		"Se puede crear una pokedex vacia para probar inserciones");

	struct pokemon *p1 = pokemon_crear_para_test(1, "poke1");
	pa2m_afirmar(p1 != NULL, "Se crea un pokemon de prueba");

	pa2m_afirmar(pokedex_insertar(pokedex, p1),
		     "Insertar pokemon valido en pokedex vacia devuelve true");
	...
	pa2m_afirmar(buscado == p1,
		     "Se encontro el pokemon insertado por nombre");

	struct pokemon *p2 = pokemon_crear_para_test(2, "poke2");
	pa2m_afirmar(p2 != NULL, "Se crea un pokemon de prueba");

	pa2m_afirmar(
		pokedex_insertar(pokedex, p2),
		"Insertar pokemon valido en pokedex ya no vacia devuelve true");
...
void pruebas_busqueda_con_pokedex()
{
	...
	pa2m_afirmar(pokedex != NULL,
		     "Se crea una pokedex con 3 pokemones de prueba");

	struct pokemon *buscado = pokedex_buscar_por_nombre(pokedex, "Pikachu");
	pa2m_afirmar(buscado->id == 1 &&
			     strcmp(buscado->nombre, "Pikachu") == 0,
		     "Buscar por nombre devuelve pokemon correcto");

	buscado = pokedex_buscar_por_nombre(pokedex, "Charmander");
	pa2m_afirmar(buscado->id == 2 &&
			     strcmp(buscado->nombre, "Charmander") == 0,
		     "Buscar por nombre devuelve pokemon correcto");

	buscado = pokedex_buscar_por_nombre(pokedex, "Bulbasaur");
	pa2m_afirmar(buscado->id == 3 &&
			     strcmp(buscado->nombre, "Bulbasaur") == 0,
		     "Buscar por nombre devuelve pokemon correcto");

	buscado = pokedex_buscar_por_id(pokedex, 1);
	pa2m_afirmar(strcmp(buscado->nombre, "Pikachu") == 0,
		     "Buscar por id devuelve pokemon correcto");

	buscado = pokedex_buscar_por_id(pokedex, 2);
	pa2m_afirmar(strcmp(buscado->nombre, "Charmander") == 0,
		     "Buscar por id devuelve pokemon correcto");

	buscado = pokedex_buscar_por_id(pokedex, 3);
	pa2m_afirmar(strcmp(buscado->nombre, "Bulbasaur") == 0,
		     "Buscar por id devuelve pokemon correcto");
...
void pruebas_pokedex_busqueda_inexistente()
{
	...
	struct pokemon *buscado =
		pokedex_buscar_por_nombre(pokedex, "poke inexistente");
	pa2m_afirmar(buscado == NULL,
		     "Buscar por nombre inexistente devuelve NULL");
...
void pruebas_pokedex_mostrar()
{
	...
	pa2m_afirmar(
		true,
		"Mostrar por nombre e id no genera errores en pokedex no vacia");
...
void pruebas_pokedex_con_cada()
{
	...
	pa2m_afirmar(aplicados == 3 && ctx_todos.contador == 3,
		     "pokedex_con_cada recorre todos los pokemones");

	contexto_hasta_limite_t ctx_limite = { 0, 2 };
	aplicados = pokedex_con_cada(pokedex, contar_hasta_limite, &ctx_limite);

	pa2m_afirmar(aplicados == 2 && ctx_limite.contador == 2,
		     "pokedex_con_cada recorre hasta el limite dado");
...
void mostrar_opcion_prueba(const char *titulo, const menu_opcion_t *opcion,
...
void pruebas_menu()
{
	...
	pa2m_afirmar(menu == NULL,
		     "menu creado con opciones NULL devuelve NULL");
	...
	pa2m_afirmar(
		menu != NULL,
		"menu crear con opciones validad devuelve un menu no NULL");
