...
void accion_buscar(void* ctx) {
    ...
    while (!salir_submenu) {
        ...
        if (opcion == 'A' || opcion == 'a') {
            ...
            struct pokemon* p = pokedex_buscar_por_nombre(contexto->pokedex, nombre);
            if (p) {
            ...
            struct pokemon* p = pokedex_buscar_por_id(contexto->pokedex, id);
            if (p) {
                printf("ID=%d, Nombre=%s, Tipo=%d, Ataque=%d, Defensa=%d, Velocidad=%d\n",
                       p->id, p->nombre, p->tipo, p->ataque, p->defensa, p->velocidad);
            } else {
...
return menu_crear("TP2 - Juego de memoria con pokemon (hecho por Jose Gonzalez Casartelli)", 
                    opciones,
                    cantidad,
                    contexto);

}
static void mostrar_opcion_estilo_simple(const char* titulo, const menu_opcion_t* opcion,
...
static void mostrar_opcion_estilo_caja(const char* titulo, const menu_opcion_t* opcion,
                                        size_t indice, void* extra)
{
    (void)extra;
...
static void mostrar_opcion_estilo_color(const char* titulo, const menu_opcion_t* opcion,
                                        size_t indice, void* extra)
{
    (void)extra;
