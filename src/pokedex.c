...
void pokedex_mostrar_por_nombre(pokedex_t *p)
{
	if (!p)
		return;
	abb_con_cada_elemento(p->abb_nombre, ABB_INORDEN, mostrar_pokemon,
			      NULL);
}
...
size_t pokedex_con_cada(pokedex_t *p, bool (*f)(struct pokemon *, void *extra),
			void *extra)
{
	if (!p || !f)
		return 0;
	...
	return abb_con_cada_elemento(p->abb_id, ABB_INORDEN, wrapper_abb, &ctx);
}
