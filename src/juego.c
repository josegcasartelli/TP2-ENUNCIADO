...
static void mostrar_tablero(const carta_t* cartas, size_t cant_cartas)
{
    ...
    for (size_t f = 0; f < filas; f++) {
        for (size_t c = 0; c < columnas; c++) {
            ...
            if (carta->eliminada) {
                printf("[%2zu] %-10s ", i, carta->pokemon->nombre);
            } else {
                printf("[%2zu] %-10s ", i, "??");
            }
        }
...
static int leer_posicion_valida(const carta_t* cartas, size_t cant_cartas, bool* salir_juego)
{
    ...
    while (!posicion_valida && !(*salir_juego)) {
        printf("Ingrese una posicion de carta (0-%zu) o 'q' para salir: ", cant_cartas - 1);
        ...
        if (!linea) {
            ...
            if ((linea[0] == 'q' || linea[0] == 'Q') && linea[1] == '\0') {
                *salir_juego = true;
            } else {
                es_numero = true;
...
void juego_jugar_con_semilla(pokedex_t* pokedex, unsigned int semilla)
{
    ...
    while (parejas_restantes > 0 && !error_memoria_jugadas && !salir_juego) {
        printf("\n======================================\n");
        ...
        int pos1 = leer_posicion_valida(cartas, cant_cartas, &salir_juego);
        ...
        if (!salir_juego) {
            ...
            while (!pos2_valida && !salir_juego) {
                pos2 = leer_posicion_valida(cartas, cant_cartas, &salir_juego);
                if (!salir_juego) {
                    if (pos2 == pos1) {
            ...
            if (!salir_juego) {
                ...
                if (cant_jugadas == cap_jugadas) {
                    ...
                    jugada_t* nuevo =
                        realloc(jugadas, nueva_cap * sizeof(jugada_t));
                    if (!nuevo) {
                ...
                if (!error_memoria_jugadas) {
                    ...
                    if (ultimas) {
                        jugada_t* copia = malloc(sizeof(jugada_t));
                        if (copia) {
                            *copia = nueva;

                            if (cola_cantidad(ultimas) == 5) {
                                jugada_t* vieja = cola_desencolar(ultimas);
                                free(vieja);
                            }

                            if (!cola_encolar(ultimas, copia)) {
                                free(copia);
                            }
                        }
                    }
